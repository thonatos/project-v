# go-workspace Specification

## Purpose
定义 monorepo 中 Go 工程的组织约定：独立的 `go/` 目录与 pnpm workspace 世界隔离、单一 `go.mod`（module path `implements.io/projectv`）的 `cmd/internal` 布局、Go 代码的格式化与静态检查约定、与 Node CI 解耦的独立 Go CI，以及基于 distroless/scratch 的最小化容器产物。
