# go-artusx-api Specification

## Purpose
定义 Go 版 `artusx-api`（基于 echo）的运行时行为：首页与静态文本路由、移除 strategies/ClickHouse 能力、webhook 的 token 鉴权与经 Telegram Bot API 的消息推送，以及配置来源、监听端口与 CORS 策略，使其行为与原 Node 版保持一致并可与之并存。
