...
import { publishOne } from '~/lib/services/publish.server';
import { getEntryByKey } from '~/lib/services/entry.server';
import { jsonOk, jsonError, readJson, handleError } from '~/lib/api.server';

import type { Route } from './+types/api.namespaces.$slug.entries.$key.publish';
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  try {
    ...
    const r = publishOne(entry.id, body.locale, body.version, ctx.user.id);
    return jsonOk(r);
  } catch (e) {
