...
import { discard } from '~/lib/services/publish.server';
import { getEntryByKey } from '~/lib/services/entry.server';
import { jsonOk, jsonError, readJson, handleError } from '~/lib/api.server';

import type { Route } from './+types/api.namespaces.$slug.entries.$key.discard';
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  try {
    ...
    const ok = discard(entry.id, body.locale, body.version, ctx.user.id);
    return jsonOk({ ok });
  } catch (e) {
