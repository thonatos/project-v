...
pull 先 `GET /snapshot/studio-ui/meta` 取语种清单(不硬编码语种),逐语种拉取文案落地为本地资源,并回写 `_meta.json`。对系统某语种尚未翻译的 key,以本地 `zh-cn` key 全集为基准**写占位**(源语言用本地源文案,其余语种用空串),保证 build 不缺 key。pull 成功后会自动刷新 `generated.ts`;占位只写本地 bundle,**不回写 studio**。
