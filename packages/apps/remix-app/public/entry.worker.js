  ...
  if (!initialMatches) {
    ...
    if (errors2) {
      ...
      initialized = initialMatches
        .slice(0, idx + 1)
        .every((m) => !shouldLoadRouteOnHydration(m.route, loaderData, errors2));
    } else {
...
async function callDataStrategyImpl(
  ...
  let loadRouteDefinitionsPromises = matches.map((m) =>
    m.route.lazy ? loadLazyRouteModule(m.route, mapRouteProperties, manifest) : void 0
  );
  let dsMatches = matches.map((match, i) => {
    ...
    let resolve = async (handlerOverride) => {
      ...
      return shouldLoad
        ? callLoaderOrAction(type, request, match, loadRoutePromise, handlerOverride, requestContext)
        : Promise.resolve({
...
async function handleRequest({
  ...
  try {
    ...
    if (
      ...
      ((_a = route == null ? void 0 : route.module) == null ? void 0 : _a.workerAction)
    ) {
